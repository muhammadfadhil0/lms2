                ...
                <div class="max-w-7xl mx-auto">
                    ...
                    <div class="mb-8">
                        <?php if (!empty($justActivated)): ?>
                            <div class="mb-4 p-4 border border-green-200 bg-green-50 text-green-700 rounded text-sm flex items-start">
                                <i class="ti ti-check mr-2 mt-0.5"></i>
                                <div>
                                    <strong>Ujian Aktif.</strong> Ujian telah dibuat dan otomatis diaktifkan. Tambahkan soal sekarang.
                                </div>
                            </div>
                        <?php endif; ?>
                        <div class="flex items-center justify-between mb-4">
                            <a href="buat-ujian-guru.php?ujian_id=<?= (int)$ujian['id'] ?>" class="group flex items-center space-x-2 hover:opacity-90" title="Kembali untuk mengubah identitas ujian">
                                <div class="w-8 h-8 bg-green-500 text-white rounded-full flex items-center justify-center text-sm font-medium group-hover:ring-2 group-hover:ring-green-300">
                                    <i class="ti ti-check text-sm"></i>
                                </div>
                                <span class="text-sm text-green-600 underline decoration-dotted group-hover:text-green-700">Identitas Ujian</span>
                            </a>
                    ...
                    <input type="hidden" id="ujian_id" value="<?= (int)$ujian['id'] ?>" data-autoscore="<?= $autoScoreFlag ? '1' : '0' ?>">
                        ...
                        <div class="lg:col-span-3 space-y-6">
                            ...
                            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                    <div>
                                        ...
                                        <p class="font-medium text-gray-800" id="exam-name-display"><?= htmlspecialchars($ujian['namaUjian']) ?></p>
                                    </div>
                                    <div>
                                        <p class="text-gray-600">Kelas:</p>
                                        <p class="font-medium text-gray-800" id="exam-class-display"><?= htmlspecialchars($ujian['namaKelas'] ?? '-') ?></p>
                                    </div>
                                    <div>
                                        <p class="text-gray-600">Mata Pelajaran:</p>
                                        <p class="font-medium text-gray-800" id="exam-subject-display"><?= htmlspecialchars($ujian['mataPelajaran']) ?></p>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <p class="font-medium text-gray-800" id="exam-date-display"><?= htmlspecialchars(date('d M Y', strtotime($ujian['tanggalUjian']))) ?></p>
                                    </div>
                                    <div>
                            ...
                            <div id="questions-container" class="space-y-4">
                                <?php if (!empty($soalList)): ?>
                                    ...
                                    ?>
                                        <div class="question-card bg-white rounded-lg shadow-sm border border-gray-200 p-6 <?php echo $idx === 1 ? 'active' : ''; ?> <?= ($autoScoreFlag && !$isMC) ? 'opacity-60 pointer-events-none relative' : ''; ?>" data-question-id="<?= $idx ?>" data-soal-id="<?= (int)$s['id'] ?>" data-original-type="<?= htmlspecialchars($tipeValue) ?>" data-active="<?= (!$autoScoreFlag || $isMC) ? '1' : '0' ?>">
                                            <!-- Question Header -->
                                            <div class="flex items-center justify-between mb-4">
                                                ...
                                                <div class="flex items-center space-x-2">
                                                    ...
                                                    <button class="delete-question p-2 text-gray-400 hover:text-red-500 transition-colors rounded-lg hover:bg-gray-50" title="Hapus Soal" data-delete-existing="1">
                                            ...
                                            <div class="mb-4">
                                                <select class="question-type-select w-full md:w-auto px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 bg-white">
                                                    ...
                                                    <option value="long_answer" <?= $tipeValue === 'long_answer' ? 'selected' : ''; ?>>Jawaban Panjang</option>
                                                </select>
                                            </div>
                                            <!-- Question Input -->
                                            <div class="mb-4">
                                                <label class="block text-sm font-medium text-gray-700 mb-2">Pertanyaan</label>
                                                <textarea class="question-text w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 resize-none" rows="3" placeholder="Masukkan pertanyaan..."><?= htmlspecialchars($s['pertanyaan']) ?></textarea>
                                            </div>
                                            ...
                                            <div class="answer-options mb-4 <?= !$isMC ? 'hidden' : ''; ?>">
                                                ...
                                                <div class="space-y-2">
                                                    <?php if ($isMC): $letters = array_keys($pilihan);
                                                        foreach ($pilihan as $opsi => $det): ?>
                                                            <div class="flex items-center space-x-3">
                                                                <input type="radio" name="correct_answer_<?= $idx ?>" value="<?= htmlspecialchars($opsi) ?>" class="text-orange-500 focus:ring-orange-500" <?= $det['benar'] ? 'checked' : ''; ?>>
                                                                <span class="w-6 text-sm font-medium text-gray-600"><?= htmlspecialchars($opsi) ?>.</span>
                                                                <input type="text" class="option-input flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500" placeholder="Pilihan <?= htmlspecialchars($opsi) ?>" value="<?= htmlspecialchars($det['teks']) ?>">
                                            ...
                                            <!-- Points Section -->
                                            <div class="points-section mt-4 pt-4 border-t border-gray-200 <?= $autoScoreFlag ? 'hidden' : '' ?>">
                                                <div class="flex items-center space-x-2">
                                                    <span class="text-sm text-gray-700">Poin:</span>
                                                    <input type="number" class="question-points w-20 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500" value="<?= $poin ?>" min="1" max="100">
                                ...
                                <?php else: ?>
                                    ...
                                    <div class="question-card bg-white rounded-lg shadow-sm border border-gray-200 p-6 active" data-question-id="1">
                                        ...
                                        <div class="mb-4">
                                            <label class="block text-sm font-medium text-gray-700 mb-2">Pertanyaan</label>
                                            <textarea class="question-text w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 resize-none" rows="3" placeholder="Masukkan pertanyaan..."></textarea>
                                        </div>
                                        <div class="mb-4">
                                            <button class="add-image-btn flex items-center space-x-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                                                <i class="ti ti-photo"></i>
                                        ...
                                        <div class="answer-key hidden mb-4">
                                            <label class="block text-sm font-medium text-gray-700 mb-2">Kunci Jawaban</label>
                                            <textarea class="answer-key-text w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 resize-none" rows="2" placeholder="Masukkan kunci jawaban..."></textarea>
                                        </div>
                                ...
                                <div class="mt-6">
                                    ...
                                    <div id="question-nav" class="grid grid-cols-5 gap-2">
                                        <?php if (!empty($soalList)): foreach ($soalList as $i => $s): $n = $i + 1; ?>
                                            ...
                                            <button class="question-nav-item flex items-center justify-center aspect-square text-sm rounded-lg border border-orange bg-orange-50 text-orange font-semibold" data-question="1" title="Soal 1">1</button>
                                        <?php endif; ?>
                                    </div>
                                </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 mt-8 pt-6 border-t border-gray-200">
                        <a href="buat-ujian-guru.php?ujian_id=<?= (int)$ujian['id'] ?>" class="px-6 py-3 border border-gray-200 text-gray-600 rounded-lg hover:bg-amber-50 transition-colors font-medium text-center">
                            Edit Identitas Ujian
                        </a>
                        <button id="preview-exam-btn" class="px-6 py-3 border border-gray-200 text-gray-600 rounded-lg hover:bg-blue-50 transition-colors font-medium">
                            Preview Ujian
                        </button>
                        <button id="save-draft-btn" class="px-6 py-3 border bg-orange text-white rounded-lg hover:bg-orange-50 transition-colors font-medium">
                            Simpan Ujian
                        </button>
                    </div>
                </div>
